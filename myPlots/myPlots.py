...
def plot_vs_pressure(x, title=None, xlabel=None, label=None, kind=None, slice=False):
    ...
    def plot(xx, l):
        ...
        if kind:
            if kind == "logx" or 'semilogx' or 'logX':
                plt.semilogx(xx, p, label=l)
            elif kind == "logy" or 'semilogy' or 'logY':
                plt.semilogy(xx, p, label=l)
            elif kind == "loglog" or 'log' or 'Log':
                plt.loglog(xx, p, label=l)
        else:
